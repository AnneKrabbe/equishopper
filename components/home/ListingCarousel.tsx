  ...
  return (
    <section className="w-full">
      <div className="w-full px-4 md:px-8">
        ...
        {listings.length === 0 ? (
          <div className="rounded-[28px] border border-[#eadfcb] bg-[#fbfaf7] px-6 py-10 text-center">
            <p className="text-sm text-stone-500">
              Der er endnu ingen annoncer her.
            </p>
          </div>
        ) : (
        <div className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-7 md:gap-5">
            {listings.map((listing) => {
              ...
              return (
               <article
  key={listing.id}
  className="group relative w-[68vw] max-w-[255px] flex-none snap-start overflow-hidden rounded-[24px] bg-[#fbfaf7] shadow-[0_10px_28px_rgba(0,0,0,0.07)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_16px_38px_rgba(0,0,0,0.10)] sm:w-[235px] lg:w-[245px]"
>
                  {listing.is_we_love && (
                    <div className="absolute left-6 top-6 z-10 rounded-full bg-[#d4af37] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-[#063f32]">
                      We Love
                    </div>
                  )}
                  ...
                  <button
                    ...
                    className="absolute right-6 top-6 z-20 flex h-9 w-9 items-center justify-center rounded-full bg-white/90 shadow-sm transition hover:bg-white"
                  >
                    {isFavorite ? (
